...
async function weeklyMeals(currentDate) {
  ...
  const mealPlanResponse = await fetch(`/api/mealPlan/week/${mondayStr}`, {
    headers: {
      'Authorization': `Bearer ${getToken()}`
    }
  });
  ...
  let mealPlanItems = [];
  if (mealPlan) {
    const itemsResponse = await fetch(`/api/mealPlan/${mealPlan.id}/items`, {
      headers: {
        'Authorization': `Bearer ${getToken()}`
      }
    });
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (item) {
        ...
        removeBtn.addEventListener('click', async () => {
          const response = await fetch(`/api/mealPlan/items/${item.id}`, {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${getToken()}`
            }
